function [daily_port]=DTMS_PPT(close_price,data,tplus1,daily_port, win_size,epsilon,a,dm)
%{
...
if tplus1<win_size+1
   ...
   closebefore=close_price((tplus1-5+1):(tplus1),:);
   closepredict=max(closebefore);
   x_tplus1 = (closepredict./close_price(tplus1,:))+a*dm(tplus1,:);%x_tplus1是t+1时刻的，a是在t时刻算的，有差1个时刻的
end
...
daily_port = daily_port+ epsilon*x_tplus1_cent/norm(x_tplus1_cent);%t+1时刻的
end

daily_port = simplex_projection_selfnorm2(daily_port,1);%t+1时刻的
